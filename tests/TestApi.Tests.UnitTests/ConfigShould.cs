...
using TestApi.Tests.UnitTests.Helpers;
...
namespace TestApi.Tests.UnitTests
{
    ...
    {
        private const string TestApiAppConfigPath = "..\\..\\..\\..\\src\\TestApi\\Web.config";
        private const string TestApiTokenPath = "..\\..\\..\\..\\src\\TestApi\\Web.config.token";
        private const string AcceptanceTestAppConfigPath = "..\\..\\..\\TestApi.Tests.AcceptanceTests\\App.config";
        private const string AcceptanceTestTokenPath = "..\\..\\..\\TestApi.Tests.AcceptanceTests\\App.config.token";
        
        [TestMethod]
        public void AppSettingsInPluginConfig_Keys_MatchTokenFile()
        {
            List<string> appSettingsKeys;
            using (AppConfig.Change(TestApiAppConfigPath))
            {
            ...
            using (AppConfig.Change(TestApiTokenPath))
            {
                // tempFileName is used for the app config during this context
        ...
        {
            var webConfigXmlDoc = XDocument.Load(TestApiAppConfigPath);
            var webConfigTokenXmlDoc = XDocument.Load(TestApiTokenPath);
        ...
        public void ConnectionStringsInPluginConfig_Names_MatchTokenFile()
        {
            ...
            using (AppConfig.Change(TestApiAppConfigPath))
            {
                ...
                foreach (ConnectionStringSettings connectionStringSetting in ConfigurationManager.ConnectionStrings)
                {
                    if ((connectionStringSetting.ElementInformation.Source != null)
                        && (connectionStringSetting.ElementInformation.Source.EndsWith(TestApiAppConfigPath)))
                    {
            ...
            using (AppConfig.Change(TestApiTokenPath))
            {
                ...
                {
                    if ((connectionStringSetting.ElementInformation.Source != null)
                        && (connectionStringSetting.ElementInformation.Source.EndsWith(TestApiTokenPath)))
                    {
